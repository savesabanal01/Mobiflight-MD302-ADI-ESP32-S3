...
void MFCustomDevice::attach(uint16_t adrPin, uint16_t adrType, uint16_t adrConfig, bool configFromFlash)
{
    ...
    ********************************************************************************** */
    ...
    if (strcmp(parameter, "MD302_ADI") == 0)
        _customType = MD302_ADI_DEVICE;

    if (_customType == MD302_ADI_DEVICE) {
        /* **********************************************************************************
            Check if the device fits into the device buffer
        ********************************************************************************** */
        if (!FitInMemory(sizeof(MD302_ADI))) {
        ...
        ********************************************************************************** */
        ...
        _myADIdevice = new (allocateMemory(sizeof(MD302_ADI))) MD302_ADI(_pin1, _pin2);
        _myADIdevice->attach(Parameter1, Parameter2);
...
void MFCustomDevice::set(int16_t messageID, char *setPoint)
{
    ...
    if (_customType == MD302_ADI_DEVICE) {
        _myADIdevice->set(messageID, setPoint);
    }
}
